...
import type { UploadProject, UploadPartner } from "@/lib/supabase/queries/upload-options";
...
// Converted from lib/mock-data.ts to real data (server-fetched options as
// props, see upload/advance/page.tsx) and a real POST /api/expenses call
// via callBackend. No receipt upload here — partner advances don't take
// one at creation (collected later at reconciliation), matching this
    ...
    try {
      ...
      const message =
        error instanceof BackendError
          ? error.message
          : "Der Vorschuss konnte nicht angelegt werden. Bitte versuchen Sie es erneut.";
      toast.error("Anlegen fehlgeschlagen", { description: message });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <PageContainer>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-card ring-1 ring-line card-shape p-6 space-y-5">
            <h2 className="font-semibold text-sm">Vorschuss</h2>
            <div>
            ...
            <div className="flex gap-3 p-4 bg-accent/70 rounded-lg text-xs text-ink">
          ...
          <div className="bg-card ring-1 ring-line card-shape p-6 sticky top-8">
            ...
            <div className="mt-6 pt-6 border-t border-line space-y-3">
              <Button type="submit" className="w-full" disabled={!isValid || submitting}>
                {submitting ? "Wird angelegt …" : "Vorschuss anlegen"}
              </Button>
